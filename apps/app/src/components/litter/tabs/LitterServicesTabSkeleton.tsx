...
interface LitterServicesTabSkeletonProps {
  isFullscreen?: boolean;
}

/**
 * Native skeleton for LitterServicesTab.
 *
 * Mirrors the service-card layout: icon + service name + price row,
 * stacked in 1-col (drawer) or 2-col (fullscreen) grid. Skips the
 * conditional "Puppies for sale" sub-section since it's only rendered
 * when the litter actually has children for sale.
 */
export function LitterServicesTabSkeleton({
  isFullscreen = false,
}: LitterServicesTabSkeletonProps) {
  const serviceCount = isFullscreen ? 4 : 3;

  return (
    <div
      className="mt-3 flex flex-col space-y-8 cursor-default"
      aria-busy="true"
      aria-live="polite"
    >
      <div
      ...
      >
        {Array.from({ length: serviceCount }).map((_, index) => (
          ...
          >
            {/* Static icon — they don't change between skeleton and real */}
