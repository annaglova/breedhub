...
const PropertyItem = ({ property }) => (
  ...
  >
    ...
    <span className="">{property.caption}</span>
...
const canApplyProperty = (
  ...
  if (configType === "field") {
    ...
    const fieldTypeRestrictions = property.tags?.filter((t) =>
      t.startsWith("field_type:")
    );
...
const handlePropertyDrop = (propertyId: string, targetConfigId: string) => {
  ...
  if (!canApplyProperty(property, targetConfig)) {
    const configType = targetConfig.type;
    alert(
      `Property "${property.caption}" cannot be applied to ${configType} configs`
    );
    return;
  }
