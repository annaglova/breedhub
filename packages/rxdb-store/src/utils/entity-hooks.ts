...
/**
 * Run post-push hooks for a child record after successful Supabase sync.
 *
 * NOTE: For denormalized parent fields (e.g. pet.titles_display, pet.timeline,
 * pet.pedigree), we use the LOCAL REBUILD pattern instead of server refresh —
 * see `breedhub-docs/general/DENORMALIZATION_PATTERN.md`. The rebuild happens
 * synchronously inside `spaceStore.createChildRecord/updateChildRecord/
 * deleteChildRecord` BEFORE this hook even runs.
 *
 * This hook is reserved for cases where local rebuild is impossible (cross-pet
 * aggregations, server-only data, etc.). Currently no-op — kept for future use.
 *
...
export async function runChildPostPushHooks(
  ...
  // No active hooks — see DENORMALIZATION_PATTERN.md
}
